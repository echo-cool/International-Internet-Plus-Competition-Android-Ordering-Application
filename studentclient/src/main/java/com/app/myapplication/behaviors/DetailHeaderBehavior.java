package com.app.myapplication.behaviors;
...
public class DetailHeaderBehavior extends AppBarLayout.Behavior implements AppBarLayout.OnOffsetChangedListener, GestureDetector.OnGestureListener {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, AppBarLayout abl, int layoutDirection) {
        ...
        if (scaleOffset > initOffset) {
            ...
            if (scaleOffset >= closeOffset) {
                ...
                tv_close.setText(mContext.getString(R.string.close));
            } else {
                tv_close.setText(mContext.getString(R.string.close));
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, AppBarLayout child, MotionEvent ev) {
        judgeDrag();
        if (!canDrag) {
            if (MotionEvent.ACTION_CANCEL != ev.getAction() && (ev.getRawX() < dhv.getLeft() || ev.getRawX() > dhv.getRight() || ev.getRawY() < dhv.getTop() || ev.getRawY() > dhv.getBottom())) {
